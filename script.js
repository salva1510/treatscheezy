...
// IN-UPDATE: Ngayon ay may display na para sa Pack at Set prices
...
// IN-UPDATE: Logic para sa Discounts sa Basket
function renderBasket() {
    ...
    for (let key in basket) {
        ...
        if (productData && productData.discounts) {
            if (item.type === 'Pack' && productData.discounts.packThreshold > 0 && item.count >= productData.discounts.packThreshold) {
                finalPrice = productData.discounts.packDiscountPrice;
                discountApplied = true;
            } else if (item.type === 'Set' && productData.discounts.setThreshold > 0 && item.count >= productData.discounts.setThreshold) {
                finalPrice = productData.discounts.setDiscountPrice;
                discountApplied = true;
            }
        }
...
function sendOrder(platform) {
    ...
    for (let k in basket) {
        ...
        let finalPrice = item.price;
        if (productData && productData.discounts) {
            if (item.type === 'Pack' && item.count >= productData.discounts.packThreshold) finalPrice = productData.discounts.packDiscountPrice;
            if (item.type === 'Set' && item.count >= productData.discounts.setThreshold) finalPrice = productData.discounts.setDiscountPrice;
        }
...
function editItem(index) {
    ...
    // Discount fields
    if(item.discounts) {
        document.getElementById('packThreshold').value = item.discounts.packThreshold;
        document.getElementById('packDiscountPrice').value = item.discounts.packDiscountPrice;
        document.getElementById('setThreshold').value = item.discounts.setThreshold;
        document.getElementById('setDiscountPrice').value = item.discounts.setDiscountPrice;
    }
...
// In-update para tumawag sa saveProduct
